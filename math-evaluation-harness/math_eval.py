...
def parse_args():
    ...
    parser.add_argument("--adaptor_path", default=None, type=str)  # Add Lora
...
def prepare_data(data_name, args):
    ...
    if not args.overwrite:
        processed_files = [f for f in os.listdir(f"{args.output_dir}/{data_name}/") if
                           f.endswith(".jsonl") and f.startswith(out_file_prefix)]
...
def main(llm, tokenizer, data_name, args):
    ...
    print("args:", "\n", args, "\n")
    print("max_func_call / [EPOCH]: ", max_func_call, "\n", "===" * 10)
    ...
    all_samples, result_json = evaluate(samples=all_samples, data_name=data_name, prompt_type=args.prompt_type,
                                        execute=True)
